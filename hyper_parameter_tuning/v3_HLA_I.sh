                  ...
                  for n_units in [16] [32] [64]; do
                    for dropout_flag in True; do
                      for p_dropout in 0.2 0.5; do
                        ...
                        sbatch --gres=gpu -t 01-10:00:00 -o st125I.log \
                                -p campus-new --job-name=st125I \
                                step125_template_HLA_I_gpu.py $enc_method $n_fold $lr $V_cdrs $CNN_flag [1,3,5,7,9] $n_filters $pl_size $strides $n_dense $n_units $dropout_flag $p_dropout 1216 2207
                      done
                    done
                  done
                done
